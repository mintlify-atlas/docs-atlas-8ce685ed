...
<ParamField path="--state" type="Path">
  Path to the `SESSION_STATE.json` file to validate. At least one of `--state`
  or `--delta` must be provided.
</ParamField>
...
It supports exactly the schema keywords used by the Canon Boundary Guard schemas:
...
If the schema contains a keyword outside this set, the validator returns an
`unsupported schema keyword` error and **fails closed** — it does not silently
